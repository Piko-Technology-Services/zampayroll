...
h1 { font-family: 'IBM Plex Sans', sans-serif; font-size: 1.3rem; margin: 0 0 .5rem; color: var(--ink); }
...
<div class="wrap">
    ...
    <p>Your loan request has been sent to HR for review. You'll receive an email once a decision has been made.</p>

    <div class="card">
        <div class="row"><span>Amount</span><span>K {{ number_format($loanRequest->amount, 2) }}</span></div>
        <div class="row"><span>Payment Plan</span><span>{{ \App\Models\LoanRequest::PAYMENT_PLANS[$loanRequest->payment_plan] ?? $loanRequest->payment_plan }}</span></div>
        @if(!empty($loanRequest->documents))
        <div class="row"><span>Documents</span><span>{{ count($loanRequest->documents) }} attached</span></div>
        @endif
