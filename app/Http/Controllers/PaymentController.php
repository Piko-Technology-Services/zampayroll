...
class PaymentController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'proof_of_payment' => ['required', 'file', 'mimes:jpg,jpeg,png,pdf', 'max:5120'], // 5MB
        ...
        try {
            ...
            if (! empty($adminAddresses)) {
                Mail::to($adminAddresses)->send(new PaymentSubmissionAdminNotification($payment));
                $payment->forceFill(['admin_notified_at' => now()])->save();
            } else {
                Log::warning('No payment_notification_addresses configured — admin notification not sent for payment #' . $payment->id);
            }
        } catch (\Throwable $e) {
            Log::error('Failed to send payment admin notification: ' . $e->getMessage());
        }

        // Confirm to the submitter that we've received their proof of payment.
        try {
            Mail::to($payment->contact_email)->send(new PaymentSubmissionConfirmation($payment));
            $payment->forceFill(['user_notified_at' => now()])->save();
        } catch (\Throwable $e) {
